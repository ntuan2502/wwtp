...
import { StatusScreen } from "@/components/station/StatusScreen";
...
export default function DynamicStationPage({
  ...
  }, [station?.lastLog?.receivedAt]);

  if (!config) return <StatusScreen type="empty" message="Trạm không hợp lệ." />;
  if (isLoading) return <StatusScreen type="loading" />;
  if (error) return <StatusScreen type="error" message={error} />;
  if (!station) return <StatusScreen type="empty" />;
